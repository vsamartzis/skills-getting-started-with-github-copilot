    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          ${details.participants.length > 0 ? `<ul class="participants">${details.participants.map(email => `<li>${email}</li>`).join('')}</ul>` : '<p class="participants no-participants">No participants yet.</p>'}
        `;
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        // Refresh the activities list to show the updated participant count
